...
namespace Lykke.Numerics
{
    ...
    public class MoneyArithmeticTests
    {
        ...
        {
            Money
                .Truncate(Money.Parse(a))
                .Should()
                .Be(b);
        }
        
        private static (Money Left, Money Right) Parse(string left, string right)
        {
            return (Money.Parse(left), Money.Parse(right));
        }
    }
}